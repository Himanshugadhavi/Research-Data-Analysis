...
library(readxl)
data <- read_excel("Anand/Raw Data.xlsx", 
                   sheet = "Anand")
View(data)
...
M=predict(fit1)
v=data.frame(M)
write_xlsx(v,"E:\\8. all pdf files\\research\\Data Analysis\\Anand\\Anand_Production.xlsx")
##############################
...
expo<-minpack.lm::nlsLM(data$Production ~ a*exp(r*data$Year), 
                        data = data, 
                        start = list(a= 2.102096e-105,r=0.1220835), 
